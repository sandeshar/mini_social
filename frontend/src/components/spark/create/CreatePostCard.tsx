...
export default function CreatePostCard() {
  ...
  const [state, action, pending] = useActionState(handleFormSubmit, { 'narrative': '', 'imageURL': null, 'isPublic': isPublic ? "true" : "false", 'feeling': '' });
    ...
    <form action={action} className="relative overflow-hidden rounded-xl bg-surface-container-lowest p-6 shadow-[0_-8px_40px_-12px_rgba(49,41,80,0.06)] md:p-8">
      ...
      <div className="mb-10 flex flex-wrap items-center gap-3">
        ...
        >
          ...
          <select className="text-xs font-medium" name="feeling">
            <option className="hidden" value={''}>Feeling</option>
      ...
      <div className="flex items-center justify-end border-t border-outline-variant/10 pt-6">
        <input type="submit" className="flex items-center space-x-2 rounded-full bg-linear-to-br from-primary to-primary-container px-10 py-3.5 font-bold text-on-primary shadow-lg shadow-primary/20 transition-all hover:shadow-primary/30 active:scale-95" disabled={pending} value="Post Perspective" />
      </div>
    </form>
  );
}
